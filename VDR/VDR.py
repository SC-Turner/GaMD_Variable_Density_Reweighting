...
def parse_args():
    ...
    parser.add_argument("--step_multi", type=str, default='True',
                        ...
                        choices=['True', 'False'])
    parser.add_argument("--xlim", type=float, help='x-axis limits for graph plotting, default uses max/min values.', default=None, nargs=2, required=False)
    parser.add_argument("--ylim", type=float, help='y-axis limits for graph plotting, default uses max/min values.', default=None, nargs=2, required=False)
    ...
    if args.mode == 'single':
        ...
        if args.conv_points_num is None:
            parser.error("--conv_points_num, convergence mode requires specification of how many cutoff values to use between defined range in --conv_points")
